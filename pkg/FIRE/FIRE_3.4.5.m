...
DataConnectionInitialize[]:=Module[{temp}, (*opens the database and starts Fermat if needed*)
    ...
    If[Head[QLink]===Symbol,
        ...
        If[QSetBucketSize[BucketSize],
            MessagePrint["You are using the new version of QLink, that's nice!"];
            MessagePrint["Bucket size set to 2^",BucketSize];
            If[DatabaseUsage>0,
                MessagePrint["QLink will use ",Round[(If[DatabaseUsage<=2,DatabaseUsage,DatabaseUsage+1]*8*(2^BucketSize))/(2^20)]," MB of RAM"];
            ];
        ];
...
SetDatabaseUsage[NewDatabaseUsage_]:=Module[{temp},
    If[NewDatabaseUsage===DatabaseUsage,ErrorPrint["No change"];Return[False]];
    If[NewDatabaseUsage<DatabaseUsage,ErrorPrint["Cannot decrease database usage"];Return[False]];
    If[NewDatabaseUsage>DatabaseUsage,ErrorPrint["Increasing database usage: ",AbsoluteTiming[
        If[Head[QLink]===Symbol,
            Check[
                QLink=Install[QLinkPath],
                ErrorPrint["Failed to run the link"];
...
Info[]:=MessagePrint["New relations: ",stepcounter," , for substitution: ", tosubstitute,", integrals considered: ",calculated,", encountered: ", needed]
...
R4HO[last_, xx_, n_] := Module[{result = {}, i},
  ones = Table[1, {n}];
  For[i = n, i > 0, i--,
   If[MemberQ[xx, i], AppendTo[result, ones]; ones[[i]] = 0];
   ];
  For[i = n, i > 0, i--,
   If[And[ones[[i]] === 1, i =!= last], AppendTo[result, ones]; 
     ones[[i]] = 0];
   ];
  If[last > 0, AppendTo[result, ones]];
  result
  ]
...
HigherPair[xx_,yy_]:=Module[{temp,xs,ys,i,xd,yd,x,y,pn,totaltemp},
    ...
    If[xsn===ysn, (*if they are in the same region*)
       ...
       ydn=Table[If[ysn[[here]]===0,0,yd[[here]]],{here,1,Length[x]}];
        ...
        If[Head[SBasisO[pn,xsn]]===SBasisO,SBasisO[pn,xsn]=R4HO[SBasisRL[pn],Flatten[Position[xsn,1]],Length[xsn]]];
...
LaportaConditions[xx_]:=Module[{pn,x,ssector,r,temp},
    ...
    If[r>0,
        ...
        ,
        If[Head[SBasisN[pn, ssector]] === List, Return[False]];
...
(*            If[ELMet[y],ELcounter++;Continue[]];*)
            ...
            If[Or[TrySymmetries[integral],If[Head[IBPSector]===Symbol,Or[TryRules[integral],TryReduction[integral],TryNewReduction[integral]],False]],
                tbd=GetTableD[y];
                ELAdd[tbd,True];
                ELcounter++;Continue[];
            ];
...
TryRestrictions[yy_]:=Module[{temp,y,pn},
    ...
    If[SBasisR[pn,SSector[pn,y]]===True,
        MyMessagePrint["Restrictions applied"];
...
TryOddRestrictions[yy_]:=Module[{temp,y},
    If[OddRestrictionsPossible[yy],
        MyMessagePrint["OddRestrictions applied"];
        MakeZeroTable[ObtainNN[yy]];
        Return[True]
    ,
        Return[False]
...
(*    sss=sss+TimeUsed[];*)
    If[Not[temp[[1]]===yy],
        MyMessagePrint["Symmetry applied"];
        Clear[Coeff];
        nn=ObtainNN[temp[[1]]];
...
TryNewReduction[yy_]:=Module[{pn,y,sector,k,eq,shift,rule,coeff,temp},
    {pn,y}=yy;
    sector=SSector[pn,y];
    If[Head[SBasisN[pn,sector]]===SBasisN,Return[False]];
    For[k=1,k<=Length[SBasisN[pn,sector]],k++,
        eq=SBasisN[pn,sector][[k]];
        If[MultiMore[pn,y*sector,eq[[1]][[1]][[2]]*sector],
            shift=y-eq[[1]][[1]][[2]];
            If[Or@@((SSector[pn,shift+##[[2]]]===sector)&/@eq[[2]]),Continue[]];
            If[Expand[eq[[1]][[1]][[1]]/.AVRulesD[shift]]===0,Contninue[]];
            eq=Join@@eq;
            rule=AVRulesD[shift];    
            eq={Expand[##[[1]]/.rule],##[[2]]+shift}&/@eq;
            coeff=eq[[1]][[1]];
            eq=Drop[eq,1];
            eq=DeleteCases[eq,{0,_}];
            eq=Select[eq,(SBasisR[pn,SSector[pn,##[[2]]]]=!=True)&];
            If[Length[eq]===0,
                MakeZeroTable[ObtainNN[yy]]
            ,
                Clear[Coeff];
                temp=Reap[
                    (Coeff[Sow[ObtainNN[{pn,##[[2]]}]]]=MyToString[-##[[1]]/coeff,InputForm])&/@eq            
                ][[2]][[1]];
                MakeTable[ObtainNN[yy],temp];
            ];
            Return[True];
        ]
    ];
...
TryReduction[yy_]:=Module[{sector,i,j,l,k,shiftlist,temp,y,c1,c2,iii,pn},
    ...
    For[k=1,k<=2,k++,
        ...
        For[i=1,i<=l,i++,
            ...
            Label[TheStep];
            If[tempelement[[1]]==yy,
                MyMessagePrint["Reducing (length: ",Length[tempelement],")"];
                c2=tempCoeff[yy];
...
NewLoadBases[x_,number_] := Module[{temp},
...
LoadStart:=Module[{temp},
    ...
    ,
        ...
        If[And[Not[Head[SBasis0L[0]]===SBasis0L],Not[NumberQ[Head[SBasis0L[0]]]]],
            ...
            If[Length[{##}]>1,ProblemNumber={##}[[2]];CopyStart[0,ProblemNumber];ClearStart[0];];
            ,
            ErrorPrint["Failed"];
...
ClearSBases[n_]:=Module[{temp},
    temp={ExampleDimension,SBasis0L,SBasis0D,SBasis0C,SBasisL,SBasisD,SBasisA,SBasisH,SBasisO,SBasisC,SBasisS,SBasisR,SBasisE,SBasisRL,SBasisM,HPI,SBasisN};
    ClearForFirst[##,n]&/@temp;
]
...
SaveData[x_] := Module[{temp},
    ...
    Save[
        x, {ExampleDimension, SBasis0L, SBasis0D, SBasis0C, SBasisL, SBasisD,
          ...
          RealRegion,RealSectors,RealSector2Number,Number2RealSector,HPI,SBasisN}];]
...
LoadData[x_] := Module[{temp},
    ...
    Clear[ExampleDimension, SBasis0L, SBasis0D, SBasis0C, SBasisL, SBasisD,
      SBasisA, SBasisH, SBasisO, SBasisC, SBasisS, SBasisR, SBasisE, SBasisRL,
       SBasisM, RULES, Burning, MaxDimension, SectorNumber, MaxRegion, RealRegion,HPI,SBasisN];
    Get[x];
...
EnumerateSectors[pn_] := Module[{temp,Done},
    ...
    If[Head[SBasisO[##[[1]], ##[[2]]]] === SBasisO,
          SBasisO[##[[1]], ##[[2]]] =
            R4HO[SBasisRL[##[[1]]], Flatten[Position[##[[2]], 1]],
              Length[##[[2]]]]] & /@ temp;
    temp = Sort[temp, HigherPair];
...
(*    sss1=sss1-TimeUsed[];*)
    result=Reap[
        ...
        ,
            ...
            Sow[NewToDigits[ToExpression[SectorNumber[{pn, ssector}]],6]];
	    (*If[ordering[[1]].dd<=2,
		    ordering=R2HO[0,Flatten[Position[ssector,-1]],Length[dd]]];*)
...
(*
#prefered          /home/sander/MyFiles/tsort/gm2-3l-prefered.m
#rules             /home/sander/MyFiles/tsort/gm2-3l.rules
*)
