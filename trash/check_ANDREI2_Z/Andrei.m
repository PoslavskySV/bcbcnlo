...
mc=1.5,mb=4.8,m=mc+mb,r=mc/m,CF=4/3,CA=3,pi=Pi,mt=174.6,R1=1,R2=1,nf=6,AL=1/137,mZ = 91.1876, $GammaZ = 2.4952, $thetaW = ArcSin[Sqrt[0.23120]],cw = Cos[$thetaW],sw = Sin[$thetaW],crosssection},
DEN[p1+p2,mZ,_]=1/(s-mZ^2);
Get[("/Users/poslavsky/Projects/redberry/bcbcnlo/src/trash/check_ANDREI2_Z/"<>state<>"/"<>scheme<>"/gamma+Z/renorm/cross-sectionNLO"<>Capitalize[scheme]<>".res")];
...
andreiTree[state_]:=Block[{s,crosssection,r=MC/(MC+MB),m=MC+MB,mZ=MZ,sw=SW,cw=CW,AL = e^2/4/Pi,AS=alphaS,R1=Sqrt[MB+MC] Sqrt[\[Pi]/3]fBc,R2=Sqrt[MB+MC] Sqrt[\[Pi]/3]fBc},
Get[("/Users/poslavsky/Projects/redberry/bcbcnlo/src/trash/check_ANDREI2_Z/"<>state<>"/larin/gamma+Z/renorm/cross-sectionNLOLarin.res")];
(*Return[Coefficient[crosssection,AS^2] AS^2/.x_DEN\[Rule]-x/.DEN[p1+p2,MZ,1]\[Rule]1/(s-MZ^2)];*)
Return[Coefficient[crosssection,AS^2] AS^2];
(*Return[Coefficient[crosssection,AS^2] AS^2];*)
]


andreiCS[ecm_,state_]:=andreiCS[ecm,state,"larin"]

andreiAmp[ecm_,state_,scheme_]:=Block[{s=ecm^2,ex,
mc=1.3,mb=4.0,m=mc+mb,r=mc/m,CF=4/3,CA=3,pi=Pi,mt=170,R1=1,R2=1,nf=6,AL=1/137,mZ = 91.1876, $GammaZ = 2.4952, $thetaW = ArcSin[Sqrt[0.23120]],cw = Cos[$thetaW],sw = Sin[$thetaW],renormAmp},
